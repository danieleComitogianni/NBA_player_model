...
box_score_2015 = pd.read_excel('../../data/boxscore_2015.xlsx')
...
box_score_2015['player_name'] = box_score_2015['player_name'].apply(clean_name)
filtered_darko_df['player_name'] = filtered_darko_df['player_name'].apply(clean_name)

merged_df_2015_2016 = pd.merge(box_score_2015,filtered_darko_df, on=['player_name','season'], how='left')
print(merged_df_2015_2016.info())
print(merged_df_2015_2016[:5])
print(merged_df_2015_2016.tail(5))

# Count rows with missing DARKO metrics
darko_cols = ['age', 'tm_id', 'dpm', 'o_dpm', 'd_dpm', 'box_odpm', 'box_ddpm', 'on_off_odpm', 'on_off_ddpm']
missing_count = merged_df_2015_2016[darko_cols].isnull().any(axis=1).sum()
print(f"Rows with missing DARKO metrics: {missing_count}")

# If there are missing values, examine which players
if missing_count > 0:
    missing_players = merged_df_2015_2016[merged_df_2015_2016[darko_cols].isnull().any(axis=1)]['player_name'].unique()
